...
import { fetchRemainingRounds } from '../services/supabase';
...
const Setup: React.FC<SetupProps> = ({ onStart }) => {
  ...
  const loadRounds = async (name: string) => {
    setIsLoadingRounds(true);
    const data = await fetchRemainingRounds(name);
    setRoundsData(data);
  ...
  const handleSaveName = async () => {
    ...
    setIsNameSaved(true);
    audio.playSuccess();
    await loadRounds(playerName.trim());
  };

  const handleStartGame = () => {
    if (!isNameSaved) { alert("يرجى تأكيد اسمك أولاً."); return; }
    ...
    <div className="w-full max-w-[1600px] mx-auto grid grid-cols-1 lg:grid-cols-12 gap-8 p-6 min-h-screen items-stretch">
        ...
        <div className="glass rounded-[2rem] p-8 shadow-2xl space-y-8 sticky top-6">
          <div className="text-center space-y-2 relative">
            ...
            <div className="flex items-center justify-center gap-2">
              <span className={`h-1.5 w-1.5 rounded-full animate-pulse ${isLoadingRounds ? 'bg-yellow-500' : 'bg-green-500'}`}></span>
              <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest">
                {isLoadingRounds ? 'Updating Stock...' : 'Connected to Stock'}
              </p>
            </div>
            <button 
          ...
          <div className="bg-slate-950/60 rounded-2xl p-5 border border-white/5 space-y-4">
            <div className="flex justify-between items-center px-1">
              <label className="text-[10px] text-slate-500 font-black uppercase">اسم اللاعب (للمخزون)</label>
              {isNameSaved && <span className="text-[9px] bg-green-500/20 text-green-400 px-2 py-0.5 rounded-full font-bold">جاهز ✓</span>}
            </div>
            <div className="flex gap-2">
              <input 
                type="text" 
                value={playerName}
                onChange={(e) => { setPlayerName(e.target.value); setIsNameSaved(false); }}
                placeholder="اكتب اسمك هنا..."
                className="flex-1 bg-slate-900 border border-slate-800 rounded-xl px-4 py-3 text-xs text-white focus:ring-1 focus:ring-yellow-500 outline-none transition-all"
              />
              <button 
                onClick={handleSaveName}
                disabled={isNameSaved && !isLoadingRounds}
                className={`px-4 rounded-xl text-[11px] font-black transition-all ${isNameSaved && !isLoadingRounds ? 'bg-slate-800 text-slate-500' : 'bg-yellow-600 text-white hover:bg-yellow-500'}`}
              >
                تأكيد
              </button>
            </div>
          ...
          <div className="space-y-4">
            <input value={team1} onChange={e => setTeam1(e.target.value)} className="w-full bg-slate-900/50 border border-slate-800 rounded-2xl px-5 py-4 text-sm text-white outline-none" placeholder="الفريق الأول" />
            <input value={team2} onChange={e => setTeam2(e.target.value)} className="w-full bg-slate-900/50 border border-slate-800 rounded-2xl px-5 py-4 text-sm text-white outline-none" placeholder="الفريق الثاني" />
          </div>

          <div className="space-y-4">
            {isEditMode ? (
              <button onClick={copyModifiedImages} className="w-full py-6 bg-indigo-600 text-white text-xl font-black rounded-3xl hover:bg-indigo-500 shadow-xl border-b-8 border-indigo-800">نسخ التعديلات 📋</button>
            ) : (
              <button
                onClick={handleStartGame}
                disabled={selectedIds.length !== 5 || !isNameSaved}
          ...
          <div className="text-center">
             <p className="text-[9px] text-slate-600">نظام المخزن المباشر (Direct Stock) v1.1</p>
          </div>
        </div>
      </div>
      ...
      <div className="lg:col-span-8 xl:col-span-9 glass rounded-[2.5rem] p-6 flex flex-col shadow-2xl">
        <div className="flex flex-col md:flex-row justify-between items-center mb-8 gap-4">
          <div>
            <h2 className="text-2xl font-black text-white italic">{isEditMode ? 'محرر الأغلفة المخصص' : 'اختر فئات اللعب'}</h2>
            <p className="text-slate-500 text-[10px] font-bold uppercase tracking-widest mt-1">يتم سحب الأسئلة من المخزن المركزي مباشرة</p>
          </div>
          {!isEditMode && (
            <div className="bg-slate-900 border border-slate-800 rounded-2xl px-6 py-2 flex items-center gap-4">
              <span className="text-slate-500 text-[9px] font-black uppercase">المحدد</span>
